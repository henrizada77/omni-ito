-- ----------------------------------------------------------------------------
-- SPRINT 16 — MODELO: CONTRATO DE EXPERIÊNCIA (COM TESTEMUNHAS)
-- ----------------------------------------------------------------------------
-- Rodar no Supabase SQL Editor. Idempotente (ON CONFLICT DO UPDATE).
--
-- Insere a versão oficial (arquivo Word enviado pelo RH) do Contrato de
-- Experiência como novo modelo em public.modelos_documentos. Diferenças em
-- relação ao modelo antigo (id 37a34651-...):
--   • endereço com "Maceió/AL"
--   • blocos de assinatura EMPREGADO(A) / EMPREGADORA
--   • campos de Testemunha + CPF
--
-- Os placeholders 'xxxx' do Word foram convertidos nas mesmas {{variáveis}}
-- que o restante do sistema já substitui no envio/assinatura:
--   {{nome}} {{cpf}} {{endereco}} {{data_admissao}} {{cargo}} {{cbo}}
--   {{atribuicoes}} {{salario}} {{salario_extenso}} {{dia}} {{mes}} {{ano}}
--
-- Fluxo já existente (nada mais a fazer no banco):
--   RH → Documentos → escolhe este modelo → "Gerar Link de Assinatura"
--   → colaborador assina em /admissao/:token
--   → sign_admission_token grava em documentos_assinados com colaborador_cpf
--   → aparece na ficha do colaborador (drawer → "Contratos Assinados").
--
-- O modelo antigo NÃO é removido. Se quiser aposentá-lo, rode manualmente:
--   delete from public.modelos_documentos
--   where id = '37a34651-789a-4c28-98ab-34ac8f1a2601';
-- ----------------------------------------------------------------------------
...
VALUES (
  'b7c8d9e0-1a2b-4c3d-9e4f-5a6b7c8d9e0f',
  'Contrato de Experiência (com Testemunhas)',
